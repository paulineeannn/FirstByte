...
def find_error(token):
    ...
    # loop i = 1 to 10
    elif token.startswith("<LOOP><WHITESPACE><IDENTIFIER><WHITESPACE><ASS_OP><WHITESPACE><INTEGER><WHITESPACE><TO><WHITESPACE><INTEGER>"):
        return "SyntaxError: Missing loop variable update expression"

    # loop i = 0 to (i ++):
    elif token.startswith("<LOOP><WHITESPACE><IDENTIFIER><WHITESPACE><ASS_OP><WHITESPACE><INTEGER><WHITESPACE><TO>"):
        return "SyntaxError: Missing loop limit value"

    # e.g. loop i = 1 10 (i ++):
    elif token.startswith("<LOOP><WHITESPACE><IDENTIFIER><WHITESPACE><ASS_OP><WHITESPACE><INTEGER>"):
        return "SyntaxError: 'to' keyword missing"

    # e.g. loop i = to 10 (i ++):
    elif token.startswith("<LOOP><WHITESPACE><IDENTIFIER><WHITESPACE><ASS_OP>"):
